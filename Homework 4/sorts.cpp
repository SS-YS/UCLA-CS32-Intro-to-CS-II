...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    // Sort the vector of pointers using the STL sort algorithm with comp as the ordering relationship.
    sort(sensorPtrs.begin(), sensorPtrs.end(), comp);

    // Using the now-sorted vector of pointers, replace each Sensor in sensors with the Sensors from auxSensors in the correct order.
